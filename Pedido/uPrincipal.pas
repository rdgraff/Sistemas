...
begin
  ...
  eUnitario.ReadOnly := False;
  eValorTotal.ReadOnly := False;
  eCodigoProduto.ReadOnly := False;
  eDescricaoProduto.ReadOnly := False;
...
procedure TFPrincipal.cdsProdutosAfterPost(DataSet: TDataSet);
begin
  eCodigoProduto.Clear;
  eDescricaoProduto.Clear;
  eQuantidade.Value := 0;
...
begin
  if (Key = 13) then
  begin
    eQuantidade.Value := cdsProdutosquantidade.Value;
    eUnitario.Value := cdsProdutosunitario.Value;
    eValorTotal.Value := cdsProdutostotal.Value;
    eCodigoProduto.Value := cdsProdutoscodigo.AsInteger;
    eDescricaoProduto.Text := cdsProdutosdescricao.AsString;
    cdsProdutos.Edit;

    eQuantidade.ReadOnly := False;
    eUnitario.ReadOnly := False;
    eValorTotal.ReadOnly := True;
    eCodigoProduto.ReadOnly := True;
    eDescricaoProduto.ReadOnly := True;
    eQuantidade.SetFocus;
  end;
  if Key = VK_DELETE then
  begin
    if (MessageDlg('Tem certeza qeu desaja excluir o produto?', mtConfirmation, mbOKCancel, 0) = mrCancel) then
      exit;
    if cdsProdutossequencia.Value > 0 then
      DataModule1.fdquery1.ExecSQL(
        'DELETE FROM PEDIDO_ITENS WHERE sequencia = :sequencia', [cdsProdutossequencia.Value]);
    cdsProdutos.Delete;
  end;
end;
